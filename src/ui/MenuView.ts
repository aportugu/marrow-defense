...
export class MenuView {
  ...
  private action(action: string): void {
    switch (action) {
      ...
      case 'tutorial': this.nav = null; this.game.begin(this.selectedLevel, true); break;
  ...
  private render(kind: MenuKind, state: GameState): void {
    ...
    const addButton = (label: string, action: string, ghost = false, mobileLabel?: string): void => {
      ...
      if (mobileLabel) button.dataset.mobileLabel = mobileLabel;
    ...
    if (kind === 'entry') {
      ...
      addButton('Show control hints', 'tutorial', true, 'Controls'); addButton('Clinical Glossary', 'howto', true, 'Glossary'); addButton('Settings', 'settings', true, 'Settings');
      card.append(actions, el('p', 'keys', 'Q/W/E build · 1–5 abilities · SPACE start wave · P pause · ESC cancel'));
    } else if (kind === 'pause') {
      card.append(el('h1', undefined, 'PAUSED')); addButton('Resume', 'resume'); addButton('Restart', 'restart', true); addButton('Clinical Glossary', 'howto', true); addButton('Settings', 'settings', true); addButton('Quit to menu', 'menu', true);
    ...
    window.requestAnimationFrame(() => { if (kind === 'glossary') card.focus(); else card.querySelector<HTMLButtonElement>('button')?.focus(); });
  }
  ...
  update(state: GameState): void {
    ...
    const opening = kind === 'entry' || kind === 'start'; this.screen.classList.toggle('opening-menu', opening); this.root.classList.toggle('menu-start', opening); this.root.classList.toggle('menu-entry', kind === 'entry');
    ...
    const key = `${kind}:${state.lastWaveReport?.wave ?? ''}`; if (key !== this.lastMenuKey) { this.lastMenuKey = key; this.render(kind, state); }
